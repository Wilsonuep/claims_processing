...
def _register_default_agents() -> None:
    ...
    # NOTE: two agents were discontinued and archived (see extras/discontinued/):
    #   - the former uam_ga2 (ReAct + DuckDuckGo web tool) -> uam_ga_web_tool_arch
    #   - the former uam_ga3 (rag_claim_decomp; RAG_MODE never non-bm25, so it was
    #     redundant with bm25_claim_decomp) -> uam_ga_rag_decomp_arch
    # The remaining agents were renumbered down to uam_ga1–uam_ga5.
    ...
    register_agent(ClaimDecompBM25Agent(model_override=MODEL))  # uam_ga3  tier 2
    register_agent(FewShotCoTAgent(model_override=MODEL))       # uam_ga4  tier 3
    register_agent(DebateCoTAgent(model_override=MODEL))        # uam_ga5  tier 3
